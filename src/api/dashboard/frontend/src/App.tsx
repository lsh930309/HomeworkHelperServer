...
const requestErrorText = (error: unknown) => error instanceof DOMException && error.name === 'AbortError' ? '요청 시간이 10초를 초과했습니다.' : error instanceof Error ? error.message : '알 수 없는 API 오류가 발생했습니다.';
...
export default function App() {
  ...
  const rangeRequest = React.useRef<AbortController | null>(null);

  React.useEffect(() => () => rangeRequest.current?.abort(), []);

  React.useEffect(() => {
    rangeRequest.current?.abort();
    rangeRequest.current = null;
    const controller = new AbortController();
    const timeout = window.setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    let active = true;
    fetchJson('/api/analytics/games', controller.signal)
      .then((body) => { if (active) setAllGames(body.games || []); })
      .catch((e) => { if (active) setError(requestErrorText(e)); })
  ...
  React.useEffect(() => {
    ...
    setError(null);
    Promise.all(['timeline','summary','patterns','sessions'].map((n) => fetchJson(`/api/analytics/${n}?${q}`, controller.signal))).then(([timeline, summary, patterns, sessions]) => {
      if (!active) return;
    ...
    }).catch((e) => { if (active) setError(requestErrorText(e)); }).finally(() => window.clearTimeout(timeout));
    return () => { active = false; window.clearTimeout(timeout); controller.abort(); };
  }, [range.start, range.end, gameId, reloadKey]);

  const quick = (name: string) => {
    rangeRequest.current?.abort();
    rangeRequest.current = null;
    const now = new Date();
    if (name === '7d') setRange({ start: fmtDate(addDays(now, -6)), end: fmtDate(now) });
    if (name === '30d') setRange({ start: fmtDate(addDays(now, -29)), end: fmtDate(now) });
    if (name === 'month') setRange({ start: fmtDate(new Date(now.getFullYear(), now.getMonth(), 1)), end: fmtDate(now) });
    if (name !== 'all') return;

    const controller = new AbortController();
    rangeRequest.current = controller;
    const timeout = window.setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    const q = new URLSearchParams(gameId !== 'all' ? { game_id: gameId } : {});
    setError(null);
    fetchJson(`/api/analytics/range?${q}`, controller.signal)
      .then((bounds: ApiRange) => {
        if (rangeRequest.current === controller) setRange(bounds);
      })
      .catch((e) => {
        if (rangeRequest.current === controller) setError(requestErrorText(e));
      })
      .finally(() => {
        window.clearTimeout(timeout);
        if (rangeRequest.current === controller) rangeRequest.current = null;
      });
  };
  if (!data) return <main className="app"><div className="card loading">{error ? <><span>API 오류: {error}</span><button onClick={() => setReloadKey((key) => key + 1)}>다시 시도</button></> : '플레이 데이터를 불러오는 중…'}</div></main>;
