...
def run_automatic_migration():
    """자동 데이터 마이그레이션을 실행합니다."""
    print("자동 데이터 마이그레이션 필요 여부 확인...")
    ...
    QMessageBox.information(None, "데이터 마이그레이션",
                            "데이터를 최신 방식으로 마이그레이션하는 중입니다.\n"
                            "잠시만 기다려주세요...")
    ...
    try:
        ...
        for sc in local_data_manager.web_shortcuts:
            if not crud.get_web_shortcut_by_id(db, sc.id):
                crud.create_process(db, schemas.WebShortcutCreate(**sc.to_dict()))
        ...
        QMessageBox.information(None, "데이터 마이그레이션 완료",
        ...
        for filename in ["managed_processes.json", "web_shortcuts.json", "global_settings.json"]:
            ...
            if os.path.exists(old_file):
                ...
                print(f"'{filename}' -> '{backup_file}'로 백업 완료")

    except Exception as e:
        print(f"데이터 마이그레이션 중 오류 발생: {e}")
        QMessageBox.critical(None, "데이터 마이그레이션 실패",
                             f"데이터 마이그레이션 중 오류가 발생했습니다.\n\n{e}\n\n"
                             "프로그램을 종료합니다. 개발자에게 문의해주세요.")
        sys.exit(1) # 오류 발생 시 앱 종료
