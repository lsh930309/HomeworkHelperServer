...
```mermaid
graph LR
    A[프론트엔드 로직<br/>UI, 입력 처리] --> B[백엔드 로직<br/>무거운 연산, YOLO, OCR]
    B -.향후 서버 분리 시 활용<br/>배포 최후반에 진행.-> C[서버 배포]
```
...
```mermaid
graph TD
    subgraph "HomeworkHelper MVP System"
        subgraph "로컬 애플리케이션"
            A[프론트엔드<br/>PyQt6/Tkinter<br/>- UI 표시<br/>- 입력 처리<br/>- 데이터 시각화]
            B[백엔드 로직<br/>독립 모듈<br/>- YOLO 추론<br/>- OCR 처리<br/>- 데이터 가공]
            C[로컬 DB<br/>SQLite<br/>- 게임 데이터<br/>- 탐지 결과<br/>- 설정 정보]
...
```mermaid
flowchart TD
    A[게임 화면 캡처<br/>Screenshot] --> B[전처리<br/>- 리사이징 640px<br/>- 정규화]
    B --> C[YOLO 추론<br/>- UI 요소 탐지<br/>- BBOX 좌표 출력]
    C --> D[BBOX 영역 크롭<br/>- 탐지된 영역 추출<br/>- 개별 이미지 생성]
    D --> E[OCR 전처리<br/>- 그레이스케일<br/>- 이진화<br/>- 노이즈 제거]
    E --> F[OCR 엔진<br/>- 텍스트 추출<br/>- 신뢰도 점수]
    F --> G[후처리<br/>- 정규표현식 파싱<br/>- 숫자 추출<br/>- 검증]
    G --> H[구조화된 데이터<br/>JSON]
```
...
```mermaid
flowchart TD
    A[비디오 녹화<br/>게임 플레이] --> B[SSIM 기반<br/>스마트 샘플링]
    B --> C[프레임 추출<br/>300-500장/30분]
    C --> D[Label Studio<br/>라벨링]
    D --> E[YOLO 형식<br/>데이터셋 변환]
    E --> F[YOLOv8 학습]
    F --> G[모델 평가 및<br/>최적화]
    G --> H[최종 모델 저장<br/>best.pt]
```
...
```mermaid
flowchart TD
    A[게임 실행 중] --> B[화면 캡처<br/>주기적 or 이벤트 기반]
    B --> C[YOLO 추론<br/>UI 요소 BBOX 탐지]
    C --> D[BBOX 영역 크롭<br/>및 전처리]
    D --> E[OCR 엔진<br/>텍스트 추출]
    E --> F[후처리 및 검증]
    F --> G[데이터 저장<br/>SQLite]
    G --> H[GUI 업데이트<br/>실시간 표시]
```
