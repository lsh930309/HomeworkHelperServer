...
Status: Shared protocol reference for the active macOS client
...
- Wake: use the macOS client-local SmartThings CLI adapter.
- Sleep/restart/shutdown: use macOS client-local OpenSSH.
...
4. Client runs SSH health with the intended key only. macOS should attempt this chain automatically after successful pairing or online recovery.
