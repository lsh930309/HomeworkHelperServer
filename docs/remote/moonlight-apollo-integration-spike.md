...
Task 3의 초기 목표는 코드 구현이 아니라 **Moonlight/Apollo 연동 방향을 고정하는 설계 spike**였고,
2026-05-24 현재는 그 결정에 따라 macOS client-local runtime binding MVP까지 구현했다.
...
- macOS 클라이언트에서 `moonlight stream <host> Desktop`, `moonlight quit <host>`, 실행 중인 Moonlight 앱 focus/terminate를 보조한다.
...
수동 Moonlight ON/OFF 버튼 흐름:

1. Moonlight가 실행 중이 아닌 상태에서는 readiness가 `ready`가 아니면 버튼을 비활성화한다.
2. ready 상태에서 ON을 누르면 binding preference를 Opt-in으로 저장하고 `Desktop` 스트림을 시작한다.
...
- 2026-05-24 기준 별도 설정 테스트 버튼 대신 popover 하단 중앙의 `Moonlight ON/OFF`
  runtime 버튼으로 수동 Desktop stream 진입/종료를 검증한다.
...
- 2026-05-24 기준 client-local `moonlight quit` + Moonlight app terminate fallback까지만
  runtime 버튼 OFF 경로에 포함했다. Sunshine/Apollo API 기반 close-app은 여전히 보류한다.
...
- [ ] 다중 디스플레이 환경에서 Accessibility 권한 허용 후 popover 화면으로 Moonlight 창이 이동하는지 확인한다.
- [ ] `Moonlight ON/OFF` 버튼으로 `Desktop` stream 시작, focus, 종료 fallback이 실제 Moonlight 앱에서 동작하는지 확인한다.
