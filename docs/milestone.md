...
flowchart TB
    subgraph devices["사용자 디바이스"]
        PC["PC Client<br/>Ground-Truth<br/>• 화면 캡처 mss<br/>• YOLO UI 탐지<br/>• OCR 숫자 인식<br/>• 정확한 자원 데이터"]
        Mobile["Mobile Client<br/>Event Logger<br/>• 게임 프로세스 감지<br/>• 터치 이벤트 선택<br/>• 타임스탬프 기록<br/>• 최소 배터리 소모"]
    end

    Cloud["Cloud Backend<br/>AI Brain<br/>🔄 데이터 동기화 + 🧠 AI 예측"]
...
```mermaid
flowchart LR
    P0[Phase 0<br/>완료] --> P1[Phase 1<br/>MVP 진행 중]
    P1 --> P2[Phase 2<br/>서버]
    P2 --> P3[Phase 3<br/>AI]
    P3 --> P4[Phase 4<br/>완성]

    P1 -.->|YOLO + OCR 엔진| Base[핵심 기술 기반]
    Base -.-> P2
    Base -.-> P3
    Base -.-> P4
```
