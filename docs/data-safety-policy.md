...
2. **부분 변경 우선**: 일부 설정만 바꾸는 화면은 `patch_settings()`로 변경 필드만 저장한다.
...
7. **라우터 직접 커밋 금지**: FastAPI/PyQt 경로는 `db.commit()`을 직접 호출하지 말고 CRUD/Beholder 함수로 위임한다. Beholder 내부 incident 처리와 CRUD 내부 커밋만 예외다.
