...
- [x] Confirm non-Windows focus helper returns `False` without scanning processes.
- [x] Logic review: focus is scheduled only after `Launcher.launch_process` reports success and retries while the game window may appear late.
- [x] Confirm Windows `user32` focus calls declare ctypes signatures while remaining best-effort.
- [ ] Windows: launch each configured game and confirm focus moves to the game window once its top-level window appears.
- [ ] Windows: verify failed or cancelled launches do not steal focus.
