...
## 2026-05-12 — Android SDK License 해소 및 APK assemble gate 전환

### 작업 범위

- 사용자가 Android SDK License 수락과 추가 도구 설치를 승인한 뒤 `sdkmanager --licenses`를 완료했다.
- `platform-tools`, `platforms;android-36`, `build-tools;35.0.0`를 설치하고 `tools/check_android_sdk_readiness.py`가 passed 상태를 보고하도록 확인했다.
- Android Compose 프로젝트의 실제 compile blocker를 해소했다.
  - `gradle.properties`에 `android.useAndroidX=true`와 Gradle JVM args 추가
  - `app/build.gradle.kts`에 Java 17 `compileOptions`와 Kotlin `jvmToolchain(17)` 추가
- `tools/verify_remote_controller.py`의 Homebrew OpenJDK 17 기본 경로를 실제 JDK home으로 보정했다.
- APK 산출 뒤 남은 adb device/emulator 부재를 license blocker와 분리하기 위해 verifier에 `--allow-android-device-blocker` gate를 추가하고 contract test를 갱신했다.
- Android emulator 설치까지 시도했으나 system image 설치가 로컬 디스크 여유 공간 부족으로 중단되어 실제 install/launch smoke는 device/emulator blocker로 남겼다.
- Android README, 구동 환경 가이드, TODO, 완료 감사 문서를 최신 SDK/build/device blocker 상태로 갱신했다.

### 자체 코드 리뷰 메모

- AndroidX/Java target 설정은 Compose 앱의 실제 Gradle compile 요구사항이므로 로컬 `local.properties`가 아니라 저장소 Gradle 설정으로 고정했다.
- `local.properties` 대신 verifier 환경변수로 SDK path를 제공해 사용자별 SDK 경로 파일을 커밋하지 않는다.
- `--allow-android-device-blocker`는 APK compile 성공 이후 device/emulator 부재만 명시 허용한다. 실제 device가 연결되면 이 flag 없이 verifier를 실행해야 한다.
- emulator package는 설치됐지만 system image는 공간 부족으로 실패했으므로, 실기기/AVD runtime 관련 완료 주장은 하지 않는다.

### 테스트/검증 결과

- `yes | JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home sdkmanager --licenses` → All SDK package licenses accepted
- `JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home sdkmanager --install "platform-tools" "platforms;android-36" "build-tools;35.0.0"` → installed
...
- `adb devices -l` → connected device 없음

### 커밋 예정 Korean Lore 메시지

```text
Android APK 빌드 gate가 license 대신 device smoke 경계를 드러낸다

Constraint: 사용자가 Android SDK License 수락과 추가 도구 설치를 승인했으므로 license blocker를 실제로 해소하고 APK compile까지 증명해야 함
Rejected: --allow-android-license-blocker 유지 | license가 해소된 뒤에도 오래된 blocker를 허용하면 Android compile regression을 숨김
Confidence: high
Scope-risk: narrow
Directive: 연결된 Android device/emulator가 준비되면 --allow-android-device-blocker 없이 verifier와 UsageStats smoke를 다시 실행할 것
