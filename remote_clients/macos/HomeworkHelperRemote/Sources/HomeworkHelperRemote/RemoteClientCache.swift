...
struct RemoteClientCache {
    ...
    static func cachedResourceIconURL(for process: RemoteProcess) -> URL? {
        let url = resourceIconFileURL(for: process)
        return FileManager.default.fileExists(atPath: url.path) ? url : nil
    }
    ...
    static func cacheIcons(for processes: [RemoteProcess], baseURL: URL) async {
        ...
        for process in processes {
            guard cachedResourceIconURL(for: process) == nil,
                  let source = remoteResourceIconURL(for: process, baseURL: baseURL) else { continue }
            do {
                let (data, response) = try await URLSession.shared.data(from: source)
                guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { continue }
                guard (http.value(forHTTPHeaderField: "Content-Type") ?? "").contains("image/png") else { continue }
                try data.write(to: resourceIconFileURL(for: process), options: [.atomic])
            } catch {
                continue
            }
        }
    ...
    static func remoteResourceIconURL(for process: RemoteProcess, baseURL: URL) -> URL? {
        guard let raw = process.progress?.resourceIconURL, !raw.isEmpty else { return nil }
        if let absolute = URL(string: raw), absolute.scheme != nil { return absolute }
        guard var components = URLComponents(url: baseURL, resolvingAgainstBaseURL: false) else { return nil }
        let path = raw.hasPrefix("/") ? raw : "/\(raw)"
        let parts = path.split(separator: "?", maxSplits: 1, omittingEmptySubsequences: false)
    ...
    private static func resourceIconFileURL(for process: RemoteProcess) -> URL {
        let safeID = process.id.replacingOccurrences(of: "[^A-Za-z0-9_.-]", with: "_", options: .regularExpression)
        return iconDirectory.appendingPathComponent("\(safeID)_resource.png")
    }
}
