...
class TailscaleBinding(private val context: Context) {
    fun inspect(): TailscaleBindingState {
        ...
        return TailscaleBindingState(
            ...
            vpnActive = vpnActive,
            message = when {
                installed && vpnActive -> "Tailscale 앱과 VPN 연결을 감지했습니다. 공개 HTTPS 직접접속에서는 선택 fallback입니다."
                installed -> "Tailscale 앱은 설치되어 있지만 VPN 활성 네트워크가 보이지 않습니다. 필요하면 앱/VPN 설정에서 수동으로 켜세요."
                else -> "Tailscale 앱이 설치되어 있지 않습니다. 공개 HTTPS 직접접속에는 필수 조건이 아닙니다."
            },
        )
    }
